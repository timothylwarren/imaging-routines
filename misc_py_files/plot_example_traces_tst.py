...
from imaging import plot_utilities 
...
def make_traces():
    ...
    for crind,crfile in enumerate(files_to_plot):
        ...
        num_frames=len(dt['mn_roi'][0])
        total_time=num_frames*TIME_BETWEEN_FRAMES
        tms=np.arange(0,total_time,TIME_BETWEEN_FRAMES)
        plt.figure()
        plt.plot(tms,dt['mn_roi'][0],'k')
        stim_tms=[9.95,19.95,29.95,39.95]
        for crtm in stim_tms:
            plt.plot([crtm,crtm+.15],[550,550],'r')
        pdb.set_trace()
